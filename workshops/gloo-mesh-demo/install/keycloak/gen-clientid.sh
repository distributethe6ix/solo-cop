...
curl -v -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X PUT -H "Content-Type: application/json" -d '{"serviceAccountsEnabled": true, "authorizationServicesEnabled": true, "redirectUris": ["'https://${1}'/callback","'https://${1}'","https://localhost:8443/callback","https://localhost:8443"]}' $KEYCLOAK_URL/admin/realms/master/clients/${id}
